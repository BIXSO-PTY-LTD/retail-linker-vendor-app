...
class AddProductController extends ChangeNotifier {
  ...
  Future<void> addProduct(BuildContext context, Product product, AddProductModel addProduct, String? thumbnail, String? metaImage, bool isAdd, List<String?> tags) async {
    ...
    ApiResponse response = await shopServiceInterface.addProduct(product,addProduct ,fields, productReturnImage, thumbnail, metaImage, isAdd, attributeList![0].active, colorImageObject, tags, _digitalProductFileName);
    if(response.response != null && response.response?.statusCode == 200) {
      _productCode.clear();
      ...
      _isLoading = false;
      ApiChecker.checkApi( response);
      showCustomSnackBarWidget(getTranslated('product_add_failed', Get.context!), Get.context!);
    }
    _isLoading = false;
    notifyListeners();
  }
